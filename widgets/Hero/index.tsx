...
const Hero = ({ block }: HeroProps = {}) => {
  ...
  return (
    <section className='h-screen max-h-screen overflow-y-clip relative flex flex-col'>
      ...
      <h1
        ref={titleRef}
        className='opacity-0 w-[56.25rem] max-md:w-full leading-[1.2em] text-[4rem] max-md:text-[2.5rem] font-black text-transparent bg-clip-text bg-(image:--color-gradient-on-dark) break-words text-wrap max-md:hyphens-auto'
      >
        {slide.title}
      </h1>
      ...
      <p
        ref={descRef}
        className='opacity-0 text-[1.125rem] max-md:text-[1.125rem] mt-[1.25rem] max-md:mt-[1.25rem] font-medium text-[var(--color-on-dark-muted)] w-[35rem] max-md:w-full max-md:line-clamp-3 break-words text-wrap'
      >
        {slide.description}
      </p>
