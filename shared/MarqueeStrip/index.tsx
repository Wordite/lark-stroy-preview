...
const MarqueeStrip = ({ items, className }: IMarqueeStripProps) => {
  ...
  const content = items.map((item, i) => (
    <p key={i} className='font-medium text-[18px] text-text-black whitespace-nowrap'>{item}</p>
  ))

  return (
    <div className={`h-[38px] relative bg-accent w-screen -translate-x-(--container-offset) ${className ?? ''}`}>
      ...
      <div ref={trackRef} className='flex items-center h-full gap-[69px] w-max'>
        <div className='flex items-center gap-[69px] shrink-0'>
          {content}
        </div>
        <div className='flex items-center gap-[69px] shrink-0'>
          {content}
